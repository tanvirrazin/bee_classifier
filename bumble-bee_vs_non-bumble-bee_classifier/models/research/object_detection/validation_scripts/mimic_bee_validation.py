...
class BeeImageDetector(object):
    ...
    def setup_model(self):
        ...
        PATH_TO_CKPT = os.path.join(self.CWD_PATH,MODEL_NAME, 'frozen_inference_graph_07_12_2021_color.pb')
    ...
    def detect(self, image_path=None):
        for node in os.listdir('./mimic_input_images'):
            mimic_bee_dir = os.path.join('./mimic_input_images', node)
            if os.path.isdir(mimic_bee_dir):
                print('{}\n-------------'.format(node))
                self.correct_prediction = 0
                self.wrong_prediction = 0

                for filename in os.listdir(mimic_bee_dir):
                    self.detect_from_file(os.path.join(mimic_bee_dir, filename))

                print("Correct: {}, Wrong: {}, Accuracy: {:.6f} %".format(
                    self.correct_prediction,
                    self.wrong_prediction,
                    100 * self.correct_prediction / (self.correct_prediction + self.wrong_prediction)
                ))
                print('\n\n')
    ...
    def detect_from_file(self, image_name=None):
        ...
        if 'not_bumblebee' in pt_classes:
            # print("{} -- Correct".format(image_name))
